...
export default function Create({ cuentas, secretarias, userSecretariaId, hoy }) {
    ...
    const { data, setData, post, processing, errors } = useForm({
        ...
        secretaria_id: userSecretariaId ?? '',
        fecha: hoy ?? '',
        cuentas: [],
    });
    ...
    const addCuenta = () => {
        setData('cuentas', [...data.cuentas, { IdCuenta: '', monto: '', cantidad: 1 }]);
    };
    ...
    const selectedIds = data.cuentas.map((c) => c.IdCuenta).filter(Boolean);
    const availableCuentas = cuentas.filter((c) => !selectedIds.includes(c.id) || data.cuentas.some((dc, i) => dc.IdCuenta === c.id));
                        ...
                        <div className="p-6 text-gray-900 dark:text-gray-100">
                            <form onSubmit={submit} className="space-y-6">
                                <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
                                    ...
                                    <div>
                                        <InputLabel htmlFor="folio" value="Folio (se genera automáticamente)" />
                                        <input
                                            id="folio"
                                            type="text"
                                            value="—"
                                                ...
                                                <div key={index} className="flex items-start gap-4 p-4 border border-gray-200 dark:border-gray-600 rounded-md bg-gray-50 dark:bg-gray-700">
                                                    <div className="flex-1">
                                                        ...
                                                        >
                                                            ...
                                                            {cuentas
                                                                .filter((c) => !selectedIds.includes(c.id) || c.id === cuenta.IdCuenta)
                                                                .map((c) => (

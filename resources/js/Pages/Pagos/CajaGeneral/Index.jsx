...
export default function Index({ ordenes, formasPago, filters: initialFilters }) {
    ...
    const handleSearch = (e) => {
        ...
        window.location.href = route('pagos.caja-general') + '?' + params.toString();
    };

    const limpiar = () => {
        window.location.href = route('pagos.caja-general');
    };
                    ...
                    <div className="overflow-hidden bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg">
                        <div className="p-6 text-gray-900 dark:text-gray-100">
                            <div className="mb-6 flex items-center justify-between">
                                <h3 className="text-lg font-medium">Órdenes de Pago</h3>
                            </div>
                                ...
                                <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                    <thead className="bg-gray-50 dark:bg-gray-700">
                                        <tr>
                                            ...
                                            <th className="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">Vigencia</th>
                                            ...
                                            <th className="px-6 py-3 text-right text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">Acción</th>
                                        </tr>
                                        ...
                                        {ordenes.data?.length > 0 ? (
                                            ordenes.data.map((orden) => {
                                                ...
                                                <tr key={orden.id} className={`hover:bg-gray-50 dark:bg-gray-700 ${vencida ? 'opacity-60' : ''}`}>
                                                    <td className="whitespace-nowrap px-6 py-4 text-sm font-medium">{orden.folio ?? ('#' + orden.id)}</td>
                                                    <td className="whitespace-nowrap px-6 py-4 text-sm max-w-[250px] truncate">{orden.nombre}</td>
                                                    <td className="whitespace-nowrap px-6 py-4 text-sm">{orden.secretaria?.nombre ?? '—'}</td>
                                                    <td className="whitespace-nowrap px-6 py-4 text-sm">{orden.fecha ?? '—'}</td>
                                                    ...
                                                    <td className="whitespace-nowrap px-6 py-4 text-right text-sm">
                                                        {orden.pagado ? (
                                                            <Link
                                                                href={route('pagos.caja-general.show', orden.pagos?.[0]?.id ?? 0)}
                                                                className="inline-flex items-center rounded-md bg-blue-600 px-3 py-1.5 text-xs font-semibold uppercase tracking-widest text-white hover:bg-blue-500"
                                                            >
                                                                Ver
                                                            </Link>
                                                        ) : vencida ? (
                                                            <span className="text-xs text-red-500 font-semibold">No pagable</span>
                                                        ) : (
                                                            <button
                                                                type="button"
                                                                onClick={() => openPagarModal(orden)}
                                                                className="inline-flex items-center rounded-md bg-green-600 px-3 py-1.5 text-xs font-semibold uppercase tracking-widest text-white hover:bg-green-500"
                                                            >
                                                                Pagar
                                                            </button>
                                                        )}

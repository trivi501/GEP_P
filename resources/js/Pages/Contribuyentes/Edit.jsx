...
export default function Edit({
    ...
    const { data, setData, patch, processing, errors } = useForm({
        ...
        curp_contribuyente: contribuyente.curp_contribuyente ?? '',
        telefono: contribuyente.telefono ?? '',
        correo_electronico: contribuyente.correo_electronico ?? '',
        nombre_vialidad: contribuyente.domicilio?.nombre_vialidad ?? '',
        num_exterior: contribuyente.domicilio?.num_exterior ?? '',
        num_interior: contribuyente.domicilio?.num_interior ?? '',
        colonia: contribuyente.domicilio?.colonia ?? '',
        id_pais: contribuyente.domicilio?.id_pais ?? '',
        id_estado: contribuyente.domicilio?.id_estado ?? '',
        id_municipio: contribuyente.domicilio?.id_municipio ?? '',
        codigo_postal: contribuyente.domicilio?.codigo_postal ?? '',
        fact_id_regimen_fiscal:
            contribuyente.datos_facturacion?.[0]?.id_f4_c_regimenfiscal ?? '',
        activo: contribuyente.activo ?? false,
    });

    const submit = (e) => {
        e.preventDefault();
        patch(route('contribuyentes.update', contribuyente.id_contribuyente));
    };

...
<head>
    <meta charset="utf-8">
    <title>Comprobante de Pago Multi Predial</title>
    <style>
        @page { margin: 10mm; }
...
<body>
    ...
    @foreach($pagos as $pago)
        @php
            $predio = $pago->predio;
            $cuentasPagos = $pago->cuentasPagos;
            $ultimoAno = $pago->incidencia?->año_ultimo_pago_anterior ? (int) $pago->incidencia->año_ultimo_pago_anterior : ($predio?->año_ultimo_pago ? (int) $predio->año_ultimo_pago : (int) date('Y') - 1);
        ...
        <div class="recibo-original">
            <div class="section">
                <table width="100%">
                    ...
                    <tr>
                        <td class="value" width="80%" style="font-weight: bold; font-size: 9pt;"><b>{{ $predio?->contribuyente?->nombre ?? $pago->nombre }}</b></td>
                ...
                <table class="info-table">
                    <tr>
                        <table>
                            ...
                            <tr>
                            <tr>
                                <td class="value" style="width:100px; font-size: 10pt;"><b>{{ $predio?->contribuyente?->cuenta ?? '—' }}</b></td>
                                <td class="value" style="width:100px; font-size: 10pt;"><b>{{ $predio?->Clave_predial ?? '—' }}</b></td>
                    ...
                    <tr>
                        <table>
                            ...
                            <tr>
                            <tr>
                                <td class="value" style="width:100px">{{ $predio?->datosUrbano?->zonaUrbana?->descripcion ?? '—' }}</td>
                                <td class="value" style="width:100px">{{ $predio?->tipoPredio?->Tipo_predio ?? $predio?->tipoPredio?->nombre ?? '—' }}</td>
                                <td class="value" style="width:100px">{{ number_format($predio?->superficie ?? 0, 2) }} m²</td>
                                <td class="value" style="width:100px">{{ number_format($predio?->construccion ?? 0, 2) }} m²</td>
                            </tr>
                        </table>
                    </tr>
                </table>
                <table>
                    ...
                    <tr>
                        <td class="value" style="width:100%" style="font-size: 9pt;"><b>{{ $predio?->ubicacion_completa ?? '—' }}</b></td>
                    </tr>
                </table>
            </div>
                ...
                <table class="info-table">
                    <tr>
                        <table>
                            ...
                            <tr>
                            <tr>
                                <td class="value" style="width:100px; font-size: 10pt;">{{ $predio?->contribuyente?->cuenta ?? '—' }}</td>
                                <td class="value" style="width:100px; font-size: 10pt;">{{ $predio?->Clave_predial ?? '—' }}</td>
                                <td class="value" style="font-size: 8pt; width: 500px; text-align: right;"><b>{{ $ultimoAno + 1 == $anioActual ? $anioActual : ($ultimoAno + 1) . ' - ' . $anioActual }}</b></td>
                            </tr>
                        </table>
                    </tr>

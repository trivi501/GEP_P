        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form method="POST" action="{{ route('cajeros.update', $cajero) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-4">
                            ...
                            <select name="usuario_id" id="usuario_id" class="w-full rounded border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300" required>
                                ...
                                @foreach ($usuarios as $usuario)
                                    <option value="{{ $usuario->id }}" {{ old('usuario_id', $cajero->usuario_id) == $usuario->id ? 'selected' : '' }}>{{ $usuario->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="caja_id" id="caja_id" class="w-full rounded border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300" required>
                                ...
                                @foreach ($cajas as $caja)
                                    <option value="{{ $caja->id }}" {{ old('caja_id', $cajero->caja_id) == $caja->id ? 'selected' : '' }}>{{ $caja->nombre }} ({{ $caja->numero }})</option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-4">
                            ...
                            <div class="mt-1">
                                <label class="inline-flex items-center">
                                    <input type="radio" name="status" value="1" class="border-gray-300 dark:border-gray-600 dark:bg-gray-700 text-indigo-600 shadow-sm focus:ring-indigo-500" {{ old('status', $cajero->status) == 1 ? 'checked' : '' }}>
                                    <span class="ml-2 text-sm">Activo</span>
                                </label>
                                <label class="inline-flex items-center ml-4">
                                    <input type="radio" name="status" value="0" class="border-gray-300 dark:border-gray-600 dark:bg-gray-700 text-indigo-600 shadow-sm focus:ring-indigo-500" {{ old('status', $cajero->status) === '0' ? 'checked' : '' }}>
